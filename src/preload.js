...
contextBridge.exposeInMainWorld("mainAPI", {
  onReadyFromMain: (callback) =>
    ipcRenderer.on("ready-from-main", (_event, value) => callback(value)),
  onSongTest: (callback) =>
    ipcRenderer.on("song-test", (_event, value) => callback(value)),
  onSongAdded: (callback) =>
    ipcRenderer.on("add-song-to-main", (_event, value) => callback(value)),
  onImageAdded: (callback) =>
    ipcRenderer.on("add-image-to-main", (_event, value) => callback(value)),
  onCL2: (callback) =>
    ipcRenderer.on("CL2", (_event, value) => callback(value)),
...
contextBridge.exposeInMainWorld("displayAPI", {
  onReadyFromMain: (callback) =>
    ipcRenderer.on("ready-from-main", (_event, value) => callback(value)),
  onGetDisplayText: (callback) =>
    ipcRenderer.on("display-text-to-display", (_event, value) =>
      callback(value),
    ),
  onGetDisplayImage: (callback) =>
    ipcRenderer.on("display-image-to-display", (_event, value) =>
      callback(value),
    ),
  onGetLogo: (callback) =>
    ipcRenderer.on("logo-to-display", (_event, value) => callback(value)),
  onGetBlack: (callback) =>
    ipcRenderer.on("black-to-display", (_event, value) => callback(value)),
  onClearDisplay: (callback) =>
    ipcRenderer.on("clear-display", (_event, value) => callback(value)),
});
...
contextBridge.exposeInMainWorld("mss", {
  ...
  Section: () => {
    return new MSS.Section();
  },
  Verse: () => {
    return new MSS.Verse();
  },
  BasicSong: () => {
    return new MSS.BasicSong();
  },
  BasicSection: () => {
    return new MSS.BasicSection();
  },
});

contextBridge.exposeInMainWorld("editorAPI", {
  sendSongString: (data) => ipcRenderer.send("song-string-to-main", data),
  sendReadSong: (data) => ipcRenderer.send("editor-read-song-to-main", data),

    onReadSong: (callback) =>
      ipcRenderer.on("read-song-to-editor", (_event, value) => callback(value)),
});
